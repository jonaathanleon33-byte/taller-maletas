...
import { actualizarRecibidoPorOrden } from "@/app/ordenes/[id]/actions";

export function RecibidoPorRow({
  ordenId,
  recibidoPor,
  tecnicos,
}: {
  ...
  const [valor, setValor] = useState(recibidoPor ?? "");
  const [pending, startTransition] = useTransition();

  function cancelar() {
    setValor(recibidoPor ?? "");
    setEditando(false);
  }
  ...
  function guardar(next: string) {
    ...
    startTransition(async () => {
      await actualizarRecibidoPorOrden(ordenId, next);
      setEditando(false);
    });
  }
    ...
    <div className="flex items-center justify-between gap-4 py-2 text-sm">
      ...
      <span className="flex items-center gap-1.5 text-right font-medium text-slate-900">
        ...
        <button
          ...
          aria-label="Editar quién recibió la maleta"
